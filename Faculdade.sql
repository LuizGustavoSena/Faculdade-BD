...
(	
	...
	CONSTRAINT pkDisciplinaNomeDisciplina PRIMARY KEY(sigla),
	CONSTRAINT nomeDisciplinaNomeCursoUnico UNIQUE (nome_disciplina, nome_curso)
);
...
(
	...
	CONSTRAINT fkPossuiSigla FOREIGN KEY(sigla) REFERENCES Disciplina(sigla),
	CONSTRAINT pkPossuiRaNomeDisciplina PRIMARY KEY(ra,sigla,semestre, ano)
);
-- REMOÇÃO DO TRIGGER
...
BEGIN
	...
	SELECT @RA = ra, @SIGLA = sigla,@SEMESTRE = semestre ,@FAlTA = falta,@ANO = ano, @NOTA1 = nota_b1 , @NOTA2 = nota_b2, @SUB = sub
	FROM INSERTED
	...
	IF @NOTA1 > @NOTA2 -- NOTA 2 É A MENOR
		...
		WHERE ra = @RA AND sigla = @SIGLA  AND semestre = @SEMESTRE AND ano = @ANO
...
INSERT INTO Possui(ra,sigla,ano,semestre,falta,nota_b1,nota_b2)
VALUES(1,'PDB',2020,2,0,7,8)
...
UPDATE Possui
SET sub = 6
WHERE ra = 2 AND semestre = 2 AND ano = 2019 AND sigla = 'ING  '
...
UPDATE Possui
SET nota_b1 = 1
WHERE ra = 2 AND semestre = 1 AND ano = 2020 AND nome_disciplina = 'Inglês'
...
VALUES('ED', 'Estrutura de Dados',30,'ADS')
...
ON d.sigla = 'PDB' AND d.sigla = p.sigla -- PODE ESCOLHER POR SIGLA OU POR NOMEDISCIPLINA
...
ON p.semestre = 2  AND p.sigla = c.sigla
WHERE l.nome_aluno = 'Luiz Sena' AND p.ano = 2019 AND p.ra = l.ra
...
FROM Aluno l ,Disciplina c JOIN Possui p
ON p.situacao = 'Reprovado por falta' AND p.sigla = c.sigla
...
JOIN  Disciplina c ON c.sigla = p.sigla
